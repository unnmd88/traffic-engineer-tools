...
4. **Метрики — seed**: при пересоздании (рестарт после падения / update) берутся из репо, поэтому счётчики/латентность не сбрасываются. Ручной `start` сбрасывает счётчик (новая сессия) — поэтому `limit` считается за запуск, а не за жизнь задачи.
